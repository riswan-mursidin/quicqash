...
if(isset($_POST['update_pin'])){
  $oldpin = $_POST['oldpin'];
  $newpin = password_hash($_POST['newpin'], PASSWORD_DEFAULT);
  if(password_verify($oldpin, $rowlogin['pin_transaksi_member'])){
    $query = "UPDATE member_fina SET pin_transaksi_member='$newpin' WHERE username_member='$username'";
    $result = mysqli_query($conn, $query);
    if($result){
      $success_status = "Change Pin is successfuly";
    }
  }else{
...
<html lang="en">
  ...
  <body data-sidebar="dark">
              ...
              <?php } ?>
                <div class="card">
                  <form class="card-body" action="<?= htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    <div class="row mb-3">
                      <label for="oldpin" class="col-sm-2 col-form-label">Old Pin</label>
                      <div class="col-sm-6">
                        <input type="password" name="oldpin" class="form-control" id="oldpin">
                      </div>
                    </div>
                    <div class="row mb-3">
                      <label for="newpin" class="col-sm-2 col-form-label">New Pin</label>
                      <div class="col-sm-6">
                        <input type="password" name="newpin" class="form-control" id="newpin">
                      </div>
                    </div>
                    <div class="row mb-3">
                      <label for="confirmnewpin" class="col-sm-2 col-form-label">Confirm New Pin</label>
                      <div class="col-sm-6">
                        <input type="password" class="form-control" id="confirmNewpin">
                      </div>
                    </div>
                    <button class="btn btn-primary" name="update_pin">Update</button>
                  </form>
                </div>
              </div>
            </div>
    ...
    <script>
        ...
        var confirm_password = document.getElementById("confirmNewpin");

        function validatePassword(){
            if(password.value != confirm_password.value) {
                confirm_password.setCustomValidity("Pin Don't Match");
            } else {
